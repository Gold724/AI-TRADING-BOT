...
This script automates interactions with Bulenox ProjectX's trading platform using Playwright.
It handles secure login, trade order placement, network interception, and code generation.

Features:
- Secure login with environment variables BULENOX_USERNAME and BULENOX_PASSWORD
- Time Sync Warning modal detection and handling during login
- Dashboard confirmation with multiple selectors and retry logic
- Trading page navigation and interface readiness confirmation
- Trade order placement with ORDER tab, DOM tab, and generic selector fallbacks
- Network request interception to capture all POST requests
- Trade execution detection by parsing JSON post data for keywords
- cURL command generation and save to 'trade.sh' file
- Automatic conversion to Python requests code using 'curlconverter' package
- Screenshot capture on critical failures for debugging
- Verbose console logs at every step for traceability
- Retry element waits up to 3 times with 2 seconds delay
...
class TradeBotSentinelAutomation:
    """Advanced TradeBot Sentinel Automation Agent for Bulenox ProjectX"""
    ...
    def __init__(self, headless: bool = True, debug: bool = False):
        ...
        if not self.username or not self.password:
            print("WARNING: BULENOX_USERNAME and BULENOX_PASSWORD environment variables not set")
            print("Please set them before running the script")
        
        # Login selectors with robust fallbacks
        ...
        # Order placement selectors with comprehensive fallbacks
        ...
        self.time_sync_selectors = [
            ...
            '.notification:has-text("sync")'
        ]
        
        # Trade detection keywords for network interception
        self.trade_keywords = [
            'symbol', 'amount', 'price', 'order', 'trade',
            'buy', 'sell', 'quantity', 'side', 'type',
            'market', 'limit', 'stop', 'execute', 'position'
        ]
        self.trade_requests: List[Dict] = []
