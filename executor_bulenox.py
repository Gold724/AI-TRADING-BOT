...
class BulenoxExecutor:
    ...
    def execute_trade(self, driver, debug=False):
        ...
        try:
            logger.info(
                f"Executing trade: {self.signal['symbol']} {self.signal.get('direction', 'buy')}"
            )
            ...
            # Execute the trade
            update_heartbeat_status(
                f"🚀 Placing {self.signal.get('direction', 'buy')} order..."
            )
            ...
            if success:
                update_heartbeat_status(
                    f"✅ Trade executed successfully: {self.signal['symbol']} {self.signal.get('direction', 'buy')}"
                )
            else:
                update_heartbeat_status(
                    f"❌ Trade execution failed: {self.signal['symbol']} {self.signal.get('direction', 'buy')}"
                )
        ...
        try:
            ...
            symbol_search = WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable(
                    (By.CSS_SELECTOR, "input[placeholder='Search Symbol']")
                )
            )
        ...
        try:
            ...
            if "sl" in self.signal:
                sl_input = driver.find_element(
                    By.CSS_SELECTOR, "input[name='stopLoss']"
                )
                sl_input.clear()
            ...
            if "tp" in self.signal:
                tp_input = driver.find_element(
                    By.CSS_SELECTOR, "input[name='takeProfit']"
                )
                tp_input.clear()
            ...
            update_heartbeat_status(
                f"❌ Error setting trade parameters: {str(e)[:50]}..."
            )
            return False
        ...
        try:
            ...
            try:
                confirm_button = WebDriverWait(driver, 3).until(
                    EC.element_to_be_clickable(
                        (By.CSS_SELECTOR, ".confirm-trade-button")
                    )
                )
            ...
            success_message = WebDriverWait(driver, 5).until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, ".trade-success-message")
                )
            )

            logger.info(
                f"Trade placed successfully: {self.signal['symbol']} {direction}"
            )
            update_heartbeat_status(
                f"✅ Trade placed successfully: {self.signal['symbol']} {direction}"
            )
    ...
    def _log_trade(self, success, error=None):
        ...
        try:
            ...
            logger.info(
                f"Trade logged: {self.signal['symbol']} {self.signal.get('direction', 'buy')}"
            )
...
def execute_trade(driver, signal, session_id=None, debug=False):
    ...
    """
    update_heartbeat_status(
        f"🔄 Preparing to execute trade: {signal['symbol']} {signal.get('direction', 'buy')}"
    )
