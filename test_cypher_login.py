...
def test_cypher_login():
    ...
    try:
        ...
        if health_result2 and login_time < 10:  # Assuming cookie login should be faster
            ...
            logger.warning(
                "⚠️ Test 2 INCONCLUSIVE: Cookie login may not be working optimally"
            )
    except Exception as e:
    ...
    try:
        ...
        if any(url_part in driver.current_url for url_part in dashboard_urls):
            ...
            logger.warning(
                f"Not logged in automatically, current URL: {driver.current_url}"
            )
            logger.warning(
                "⚠️ Test 3 INCONCLUSIVE: Chrome profile may not contain login session"
            )
