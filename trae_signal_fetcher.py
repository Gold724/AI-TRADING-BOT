...
def get_next_signal():
    ...
    try:
        ...
        update_heartbeat_status(
            f"🔄 Checking for signals from {SIGNAL_SOURCE} source..."
        )
        ...
        if signal:
            update_heartbeat_status(
                f"🔔 Signal received: {signal.get('symbol')} {signal.get('direction')}"
            )
        else:
...
def _get_signal_from_file():
    ...
    try:
        ...
        for signal in signals:
            ...
            if "id" not in signal:
                signal["id"] = (
                    f"{signal.get('symbol', 'UNKNOWN')}-{signal.get('direction', 'UNKNOWN')}-{int(time.time())}"
                )
...
def _get_signal_from_api():
    ...
    try:
        ...
        if "id" not in data:
            data["id"] = (
                f"{data.get('symbol', 'UNKNOWN')}-{data.get('direction', 'UNKNOWN')}-{int(time.time())}"
            )
