...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("auto_trade_demo.log"), logging.StreamHandler()],
)
...
def execute_demo_trade():
    ...
    # Note: We're not passing profile_path as the ai_login_bulenox function
    # reads it directly from environment variables and uses AI-powered login
    ...
    try:
        logger.info(
            f"Attempting to execute trade: {trade_data} (Futures symbol: {futures_symbol})"
        )
        ...
        try:
            symbol_input = wait.until(
                EC.element_to_be_clickable((By.XPATH, selectors["symbol"][1]))
            )
            symbol_input.clear()
            ...
            executor.driver.save_screenshot(
                f"logs/screenshots/symbol_error_{timestamp}.png"
            )

        # Enter quantity
        try:
            quantity_input = wait.until(
                EC.element_to_be_clickable((By.XPATH, selectors["quantity"][1]))
            )
            ...
            executor.driver.save_screenshot(
                f"logs/screenshots/quantity_error_{timestamp}.png"
            )
        ...
        if trade_data.get("stop_loss"):
            try:
                sl_input = wait.until(
                    EC.element_to_be_clickable((By.XPATH, selectors["stop_loss"][1]))
                )
                sl_input.clear()
                ...
                executor.driver.save_screenshot(
                    f"logs/screenshots/sl_error_{timestamp}.png"
                )
        ...
        if trade_data.get("take_profit"):
            try:
                tp_input = wait.until(
                    EC.element_to_be_clickable((By.XPATH, selectors["take_profit"][1]))
                )
                tp_input.clear()
                ...
                executor.driver.save_screenshot(
                    f"logs/screenshots/tp_error_{timestamp}.png"
                )

        # Take screenshot before submitting
        executor.driver.save_screenshot(
            f"logs/screenshots/pre_submission_{timestamp}.png"
        )
        ...
        try:
            ...
            executor.driver.save_screenshot(
                f"logs/screenshots/button_error_{timestamp}.png"
            )
