...
def remove_null_bytes(file_path):
    ...
    try:
        ...
        print_colored(
            f"Fixed file: {file_path} (Backup created at {backup_path})", GREEN
        )
        return True
    except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(
        description="Find and fix null bytes in Python files"
    )
    ...
    print_colored(
        "This script will find files with null bytes that can cause issues", YELLOW
    )
    print()
    ...
    if not files_with_null_bytes:
        ...
        print_colored(
            f"Found {len(files_with_null_bytes)} files with null bytes:", YELLOW
        )
        ...
        if fix_files:
            ...
            print()
            print_colored(
                f"Fixed {fixed_count} out of {len(files_with_null_bytes)} files", GREEN
            )
