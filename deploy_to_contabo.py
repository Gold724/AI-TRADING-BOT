...
def parse_args():
    ...
    parser.add_argument(
        "--chrome-profile", help="Path to Chrome profile directory to upload (optional)"
    )
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                ...
                print_colored(
                    f"❌ Failed to connect to {ip} after {max_retries} attempts: {e}",
                    "RED",
                )
                sys.exit(1)
...
def setup_vps(client):
    ...
    if not script_path.exists():
        print_colored(
            "❌ sentinel_vps_setup.sh not found in the current directory", "RED"
        )
        return False
...
def upload_project_files(client, env_file, chrome_profile=None):
    ...
    if chrome_profile:
        ...
        if not upload_directory(client, chrome_profile, chrome_profile_dir):
            print_colored(
                "⚠️ Failed to upload Chrome profile. Continuing without it.", "YELLOW"
            )
...
def install_dependencies(client):
    ...
    exit_status = execute_command(
        client, "cd /root/ai-trading-sentinel && pip3 install -r requirements.txt"
    )
...
def start_trading_sentinel(client):
    ...
    execute_command(
        client,
        "tmux kill-session -t trading_sentinel 2>/dev/null || true",
        display=False,
    )
    ...
    execute_command(
        client,
        'tmux new-session -d -s trading_sentinel "cd /root/ai-trading-sentinel && python3 heartbeat.py"',
    )

    # Check if the session is running
    exit_status = execute_command(
        client, "tmux has-session -t trading_sentinel", display=False
    )
    ...
    print_colored(
        "✅ AI Trading Sentinel started successfully in tmux session 'trading_sentinel'",
        "GREEN",
    )
    return True
...
def main():
    ...
    if (
        setup_vps(client)
        and upload_project_files(client, args.env_file, args.chrome_profile)
        and install_dependencies(client)
        and configure_environment(client)
        and start_trading_sentinel(client)
    ):
        print_colored("\n====================================", "HEADER")
