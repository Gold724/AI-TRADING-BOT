    ...
    try:
        ...
        if captcha_elements:
            ...
            for i, elem in enumerate(captcha_elements):
                logger.warning(
                    f"CAPTCHA element {i+1}: id={elem.get_attribute('id')}, class={elem.get_attribute('class')}, name={elem.get_attribute('name')}"
                )
        ...
        for by_method, selector in username_selectors:
            try:
                ...
                logger.info(
                    f"Element attributes: id={element.get_attribute('id')}, name={element.get_attribute('name')}, type={element.get_attribute('type')}"
                )
        ...
        if not username_field:
            ...
            try:
                ...
                for i, elem in enumerate(form_elements):
                    logger.info(
                        f"Form element {i+1}: id={elem.get_attribute('id')}, name={elem.get_attribute('name')}, type={elem.get_attribute('type')}"
                    )
            except:
        ...
        for by_method, selector in submit_selectors:
            try:
                ...
                logger.info(
                    f"Element attributes: id={element.get_attribute('id')}, name={element.get_attribute('name')}, type={element.get_attribute('type')}, value={element.get_attribute('value')}, text={element.text}"
                )
        ...
        try:
            ...
            if error_elements:
                logger.warning(
                    f"Found {len(error_elements)} potential error elements before submit:"
                )
                for i, elem in enumerate(error_elements):
                    logger.warning(
                        f"Error element {i+1}: text={elem.text}, class={elem.get_attribute('class')}"
                    )
        ...
        try:
            ...
            for selector in error_selectors:
                ...
                if error_elements:
                    logger.warning(
                        f"Found {len(error_elements)} potential error elements with selector {selector}:"
                    )
                    for i, elem in enumerate(error_elements):
                        logger.warning(
                            f"Error element {i+1}: text={elem.text}, class={elem.get_attribute('class')}"
        ...
        if any(url_part in driver.current_url for url_part in dashboard_urls):
            ...
            if "login" in driver.current_url.lower():
                ...
                try:
                    ...
                    logger.info(
                        f"Username field value after submit: {username_field.get_attribute('value')}"
                    )
                    logger.info(
                        f"Password field value after submit: {password_field.get_attribute('value')}"
                    )
                    ...
                    for i, field in enumerate(hidden_fields):
                        logger.info(
                            f"Hidden field {i+1}: name={field.get_attribute('name')}, value={field.get_attribute('value')}"
                        )
                except Exception as e:
