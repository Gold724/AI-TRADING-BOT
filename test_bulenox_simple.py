...
class TestBulenoxFutures(unittest.TestCase):
    ...
    @unittest.mock.patch(
        "backend.executor_bulenox_futures.BulenoxFuturesExecutor._init_driver"
    )
    @unittest.mock.patch(
        "backend.executor_bulenox_futures.BulenoxFuturesExecutor._login"
    )
    @unittest.mock.patch(
        "backend.executor_bulenox_futures.BulenoxFuturesExecutor._place_trade"
    )
    def test_execute_trade(self, mock_place_trade, mock_login, mock_init_driver):
        ...
        executor = BulenoxFuturesExecutor(
            self.test_signal, self.stop_loss, self.take_profit
        )
        ...
        print(
            "NOTE: This will simulate a trade but NOT actually execute it in DEV_MODE."
        )
    ...
    @unittest.mock.patch(
        ...
        unittest.mock.PropertyMock(return_value="\ue006"),
    )
    ...
    def test_detect_gold_symbol(self, mock_login, mock_init_driver):
        ...
        with unittest.mock.patch.object(
            executor, "_detect_gold_symbol", return_value="GC"
        ):
    ...
    def test_trading_mode_detection(self, mock_login, mock_init_driver):
        ...
        self.assertEqual(
            result.get("adjusted_quantity"), 1
        )  # Should be limited to 1 in evaluation mode
        print(
            f"Gold quantity adjustment test: {'PASSED' if result.get('quantity_adjusted') else 'FAILED'}"
        )
