...
def login():
    try:
        ...
        if not session_id:
            session_id = (
                f"{account_id}-{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"
            )
...
def execute_trade():
    try:
        ...
        if not signal or not isinstance(signal, dict):
            return (
                jsonify(
                    {"status": "error", "message": "Invalid or missing signal data"}
                ),
                400,
            )
        ...
        if success:
            ...
            return (
                jsonify({"status": "error", "message": "Trade execution failed"}),
                500,
            )
...
def list_sessions():
        ...
        sessions.append(
            {
                ...
                "created_at": (
                    session_id.split("-")[-2] + "-" + session_id.split("-")[-1]
                    if "-" in session_id
                    else "unknown"
                ),
            }
    ...
    return (
        jsonify({"status": "success", "sessions": sessions, "count": len(sessions)}),
        200,
    )
...
def get_status():
    ...
    for session_id, executor in active_sessions.items():
        status_data["sessions"].append(
            {"session_id": session_id, "account_id": executor.account_id}
        )
...
def get_logs():
    try:
        ...
        session_id = request.args.get(
            "session_id"
        )  # Optional session ID for session logs
...
def manage_strategy():
    if request.method == "GET":
        ...
        if os.path.exists(strategy_file):
            ...
            return (
                jsonify({"status": "success", "strategy": "default", "parameters": {}}),
                200,
            )
        ...
        try:
            ...
            if not strategy_name:
                return (
                    jsonify(
                        {"status": "error", "message": "Strategy name is required"}
                    ),
                    400,
                )
            ...
            logger.info(
                f"Strategy updated to {strategy_name} with parameters {parameters}"
            )
            return jsonify(strategy_data), 200
...
# Stealth trade endpoint for direct trade execution
...
def stealth_trade():
    try:
        ...
        try:
            ...
            return jsonify({"status": "error", "message": f"Invalid JSON format: {str(json_error)}"}), 400
            
        logger.info(f"Received stealth trade request: {data}")
        
        # Extract trade parameters
        symbol = data.get("symbol")
        action = data.get("action", "BUY").upper()
        lots = data.get("lots", 1)
        broker = data.get("broker", "BULENOX")
        ...
        # Convert to signal format expected by CloudTradeExecutor
        signal = {
            "symbol": symbol,
        ...
        # Create new executor and login
        logger.info(f"Creating new session {session_id} for stealth trade")
        executor = CloudTradeExecutor(account_id=account_id, session_id=session_id)
        active_sessions[session_id] = executor
        
        executor.login()
        if not executor.driver:
            logger.error(f"Login failed for stealth trade session {session_id}")
            # Clean up failed session
            executor.close()
            del active_sessions[session_id]
            
            return jsonify({"status": "error", "message": "Login failed, cannot execute trade"}), 401
            
        # Execute trade
        logger.info(f"Executing stealth trade for session {session_id}: {signal}")
        success = executor.execute_trade(signal)
        ...
        if executor.driver:
            try:
                ...
                screenshot_path = os.path.join(screenshot_dir, f"stealth_trade_{session_id}.png")
                executor.driver.save_screenshot(screenshot_path)
                logger.info(f"Screenshot saved to {screenshot_path}")
            except Exception as e:
                logger.error(f"Failed to take screenshot: {e}")
        
        # Close session after stealth trade (don't keep it open)
        executor.close()
        del active_sessions[session_id]
        
        if success:
            logger.info(f"Stealth trade executed successfully for session {session_id}")
            return jsonify({
                "status": "success",
                "message": "Trade executed successfully",
                "session_id": session_id,
            ...
            logger.error(f"Stealth trade execution failed for session {session_id}")
            return jsonify({"status": "error", "message": "Trade execution failed"}), 500
            
    except Exception as e:
        logger.exception(f"Error during stealth trade processing: {e}")
        return jsonify({"status": "error", "message": str(e)}), 500
...
def webhook():
    try:
        ...
        # Create new executor and login
        logger.info(
            f"Creating new session {session_id} for account {account_id} from webhook"
        )
        executor = CloudTradeExecutor(account_id=account_id, session_id=session_id)
        active_sessions[session_id] = executor
        ...
        if not executor.driver:
            ...
            return (
                jsonify(
                    {"status": "error", "message": "Login failed, cannot execute trade"}
                ),
                401,
            )
                    ...
                    if len(lines) >= 3:
                        try:
                            ...
                            if "session_active" in session_data:
                                status_data["session_active"] = session_data[
                                    "session_active"
                                ]
                            ...
                            if (
                                "login successful" in status_text
                                or "trade executed" in status_text
                            ):
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Environment: {'cloud' if os.getenv('HEADLESS', 'true').lower() == 'true' else 'local'}"
    )
