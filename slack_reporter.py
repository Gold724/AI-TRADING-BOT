...
def send_slack_notification(message, notification_type=None, session_id=None, **kwargs):
    ...
    try:
        ...
        formatted_message = _format_message(
            message, notification_type, session_id, timestamp, **kwargs
        )
        ...
        update_heartbeat_status(
            f"✅ Notification sent: {notification_type or 'custom'}"
        )
        return True
...
if __name__ == "__main__":
    ...
    update_heartbeat_status(
        "🧪 Testing heartbeat status updates from slack_reporter.py"
    )
