...
def login_bulenox_with_profile(debug=False):
    ...
    try:
        ...
        update_heartbeat_status(
            "⚠️ Chrome initialization failed, trying alternative approach..."
        )
        ...
        try:
            ...
            update_heartbeat_status(
                "⚠️ ChromeDriverManager failed, trying with default Chrome..."
            )
    ...
    try:
        ...
        try:
            ...
            update_heartbeat_status(
                "🔑 Login form found, already authenticated via Chrome profile"
            )
            ...
            try:
                WebDriverWait(driver, 10).until(
                    EC.presence_of_element_located(
                        (By.CSS_SELECTOR, ".dashboard-element")
                    )
                )
                ...
                update_heartbeat_status(
                    "⚠️ Timed out waiting for dashboard to load, but might still be logged in"
                )
