...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(
                "logs", f"manual_trade_test_{datetime.now().strftime('%Y%m%d')}.log"
            )
        ),
...
def run_manual_trade_test():
        ...
        try:
            ...
            if driver:
                ...
                update_heartbeat_status(
                    f"❌ Login failed. Retrying... ({login_attempts}/{max_login_attempts})"
                )
    ...
    try:
        update_heartbeat_status(
            f"📈 Executing {test_signal['direction']} trade for {test_signal['symbol']}..."
        )
        logger.info(f"Executing trade: {test_signal}")

        # Execute the trade
        trade_result = execute_trade(
            driver, test_signal, session_id=session_id, debug=True
        )
        ...
        if trade_result.get("success", False):
            update_heartbeat_status(
                f"✅ Trade executed: {test_signal['symbol']} {test_signal['direction']}"
            )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(".env"):
        ...
        print(
            "⚠️ Created default .env file. Please update with your credentials before running again."
        )
    else:
        # Run the test
