...
from utils.github_integration import check_for_updates, pull_updates
from utils.github_integration import update_heartbeat_status as github_status_update
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join("logs", f"heartbeat_{datetime.now().strftime('%Y%m%d')}.log")
        ),
...
def run_heartbeat(debug=False, check_github_updates=True):
    ...
    if check_github_updates:
        try:
                    ...
                    if success:
                        ...
                        if os.getenv("RESTART_AFTER_UPDATE", "false").lower() == "true":
                            ...
                            github_status_update(
                                "restarting", {"reason": "update applied"}
                            )
            ...
            update_heartbeat_status(
                f"❌ Error checking for GitHub updates: {str(e)[:50]}..."
            )
    ...
    login_timeout = (
        int(os.getenv("LOGIN_TIMEOUT_MINUTES", "30")) * 60
    )  # Convert to seconds

    # Track GitHub update checks
    last_github_check_time = datetime.now()
    github_check_interval = (
        int(os.getenv("GITHUB_CHECK_INTERVAL_MINUTES", "60")) * 60
    )  # Convert to seconds
    ...
    while True:
        try:
            ...
            if (
                ...
                and (datetime.now() - last_github_check_time).total_seconds()
                >= github_check_interval
            ):
                    ...
                    if updates_available:
                        ...
                        if os.getenv("AUTO_UPDATE_GITHUB", "false").lower() == "true":
                            ...
                            if success:
                                ...
                                # Restart the application if configured
                                if (
                                    os.getenv("RESTART_AFTER_UPDATE", "false").lower()
                                    == "true"
                                ):
                                    logger.info(
                                        "Restarting application after update..."
            ...
            if not validate_signal(signal):
                ...
                update_heartbeat_status(
                    f"🚫 Signal rejected: {signal.get('symbol')} {signal.get('direction')}"
                )
                send_slack_notification(
            ...
            if (
                active_driver is None
                or last_login_time is None
                or (current_time - last_login_time) > login_timeout
            ):
                ...
                logger.info("Initializing AI-powered login to Bulenox...")
                update_heartbeat_status("🔐 Initializing AI-powered login to Bulenox...")
                active_driver = ai_login_bulenox(debug=debug)
            ...
            update_heartbeat_status(
                f"📈 Executing {signal.get('direction')} trade for {signal.get('symbol')}..."
            )
            ...
            update_heartbeat_status(
                "⚠️ Sentinel HEARTBEAT stopped (keyboard interrupt)",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-github", action="store_true", help="Disable GitHub update checks"
    )
