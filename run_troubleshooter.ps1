...
if (-not (Test-Path $troubleshooterScript)) {
    Write-ColorOutput "Error: ultimate_login_troubleshooter.py not found in directory" "Red"
...
$usernameDisplay = $env:BULENOX_USERNAME.Substring(0, [Math]::Min(3, $env:BULENOX_USERNAME.Length)) + "***"
Write-ColorOutput "   Username: $usernameDisplay" "White"
...
if ($env:VIRTUAL_ENV) {
    ...
    Write-ColorOutput "No virtual environment detected" "Yellow"
...
foreach ($dep in $dependencies) {
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-ColorOutput "$dep not installed" "Yellow"
        }
    } catch {
        Write-ColorOutput "$dep not installed" "Yellow"
    }
...
if ($installChoice -eq "y" -or $installChoice -eq "Y") {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-ColorOutput "Some dependencies may have failed to install" "Yellow"
    }
}
...
Write-Host ""
Write-ColorOutput "Starting Ultimate Login Troubleshooter..." "Green"
Write-ColorOutput "================================================================================" "Cyan"
Write-Host ""
