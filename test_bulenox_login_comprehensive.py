    ...
    try:
        ...
        for cookie in cookies:
            ...
            try:
                ...
                logger.warning(
                    f"[{session_id}] Could not add cookie {cookie.get('name')}: {str(e)}"
                )
...
def init_driver(use_profile=True, headless=False):
    ...
    try:
        ...
        if use_profile:
            try:
                logger.info(
                    f"[{session_id}] Initializing Chrome driver with user profile"
                )
                options = Options()
                ...
                logger.info(
                    f"[{session_id}] Chrome driver initialized successfully with profile"
                )
                return driver
            except Exception as e:
                logger.error(
                    f"[{session_id}] Profile-based driver init failed: {str(e)}"
                )
        ...
        try:
            logger.info(
                f"[{session_id}] Initializing Chrome driver without user profile"
            )
            options = Options()
            options.add_argument("--no-sandbox")
            options.add_argument("--disable-dev-shm-usage")
            ...
            driver.set_window_size(1366, 768)
            logger.info(
                f"[{session_id}] Chrome driver initialized successfully without profile"
            )
            return driver
        except Exception as e:
        ...
        driver = webdriver.Chrome(
            service=Service(ChromeDriverManager().install()), options=minimal_options
        )
        driver.set_window_size(1366, 768)
        ...
        logger.error(
            f"[{session_id}] All driver initialization attempts failed: {str(e)}"
        )
        return None
    ...
    try:
        ...
        if load_cookies(driver, cookie_path):
            ...
            if any(url_part in driver.current_url for url_part in dashboard_urls):
                logger.info(
                    f"[{session_id}] Successfully logged in with cookies (current URL: {driver.current_url})"
                )
                return True
            else:
                logger.info(
                    f"[{session_id}] Cookie login failed, current URL: {driver.current_url}"
                )
                return False
        else:
            logger.info(
                f"[{session_id}] No cookies available or could not load cookies"
            )
            return False
    except Exception as e:
...
def login_with_form(driver):
    ...
    try:
        ...
        if not username or not password:
            logger.error(
                f"[{session_id}] Missing Bulenox credentials in environment variables"
            )
            return False
        ...
        logger.info(
            f"[{session_id}] Attempting to find username field with {len(username_selectors)} different selectors"
        )
        for by_method, selector in username_selectors:
            try:
                ...
                logger.info(
                    f"[{session_id}] Found username field with selector: {by_method}={selector}"
                )
                break
        ...
        if not username_field:
            ...
            logger.error(
                f"[{session_id}] Page source excerpt: {driver.page_source[:500]}..."
            )
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            error_screenshot = f"screenshots/login_form_not_found_{timestamp}.png"
            driver.save_screenshot(error_screenshot)
            logger.error(
                f"[{session_id}] Login form not found screenshot saved to: {error_screenshot}"
            )
            return False
        ...
        try:
            ...
            for by_method, selector in password_selectors:
                try:
                    ...
                    logger.info(
                        f"[{session_id}] Found password field with selector: {by_method}={selector}"
                    )
                    break
            ...
            try:
                hidden_fields = driver.find_elements(
                    By.XPATH, "//input[@type='hidden']"
                )

                if hidden_fields:
                    logger.info(
                        f"[{session_id}] Found {len(hidden_fields)} hidden fields in login form"
                    )
                    for field in hidden_fields:
                        name = field.get_attribute("name")
                        value = field.get_attribute("value")
                        logger.info(
                            f"[{session_id}] Hidden field: name={name}, value={value}"
                        )
            except Exception as e:
                logger.warning(
                    f"[{session_id}] Could not check hidden fields: {str(e)}"
                )
            ...
            for by_method, selector in submit_selectors:
                try:
                    ...
                    logger.info(
                        f"[{session_id}] Found submit button with selector: {by_method}={selector}"
                    )
                    break
                except NoSuchElementException:
                    continue
            ...
            logger.info(
                f"[{session_id}] Pre-submit screenshot saved to: {pre_submit_screenshot}"
            )
            ...
            driver.save_screenshot(error_screenshot)
            logger.error(
                f"[{session_id}] Error screenshot saved to: {error_screenshot}"
            )
            return False
        ...
        try:
            ...
            logger.info(
                f"[{session_id}] Waiting for redirect to any of these URLs: {dashboard_urls}"
            )
            ...
            logger.info(
                f"[{session_id}] Successfully logged in to Bulenox (redirected to {driver.current_url})"
            )
            return True
        except TimeoutException:
            ...
            if any(url_part in current_url for url_part in dashboard_urls):
                ...
                logger.info(
                    f"[{session_id}] Successfully logged in to Bulenox (redirected to {current_url})"
                )
                return True
            else:
                logger.error(
                    f"[{session_id}] Login failed - unexpected redirect to {current_url}"
                )
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                error_screenshot = f"screenshots/bulenox_login_error_{timestamp}.png"
                driver.save_screenshot(error_screenshot)
                ...
                try:
                    ...
                    if error_messages:
                        for error in error_messages:
                            logger.error(
                                f"[{session_id}] Login error message found: {error.text}"
                            )
                except:
                    pass
                ...
                try:
                    ...
                    logger.info(
                        f"[{session_id}] Password field value after submit: {password_field.get_attribute('value')}"
                    )
                except Exception as e:
                    logger.error(
                        f"[{session_id}] Error checking form fields after submit: {str(e)}"
                    )
...
def check_site_status():
    ...
    try:
        ...
        if status_code == 200:
            logger.info(
                f"[{session_id}] Site is up and responding with status code {status_code}"
            )
            return True
        else:
            logger.warning(
                f"[{session_id}] Site responded with status code {status_code}"
            )
            return False
