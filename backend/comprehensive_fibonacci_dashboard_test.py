...
class TestFibonacciDashboard(unittest.TestCase):
    ...
    def test_07_add_signal(self):
        ...
        response = requests.post(f"{self.base_url}/api/add-signal", json=new_signal)
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_09_update_settings(self):
        ...
        response = requests.post(
            f"{self.base_url}/api/update-settings", json=new_settings
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertTrue(data["success"])
    ...
    def test_10_mock_execute_trade(self):
        ...
        response = requests.post(
            f"{self.base_url}/api/execute-trade", json=trade_params
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertTrue(data["success"])
