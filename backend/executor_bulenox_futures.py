...
class BulenoxFuturesExecutor(BaseExecutor):
    def __init__(
        self, signal, stopLoss=None, takeProfit=None, detect_symbol_on_init=False
    ):
    ...
    def _login(self, driver):
        ...
        try:
            ...
            driver.save_screenshot(
                f"{self.screenshot_dir}/bulenox_login_error_{timestamp}.png"
            )
            return False
    ...
    def _detect_gold_symbol(self, driver):
        ...
        if not symbol_input:
            print(
                "Could not find symbol search input, taking screenshot of current page"
            )
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            screenshot_path = (
                f"{self.screenshot_dir}/gold_symbol_input_not_found_{timestamp}.png"
            )
            driver.save_screenshot(screenshot_path)
        ...
        for gold_symbol in self.gold_symbols:
            try:
                ...
                try:
                    ...
                    dropdown_results = driver.find_elements(
                        By.XPATH,
                    ...
                    if dropdown_results and len(dropdown_results) > 0:
                        ...
                        if gold_symbol == "GC":
                            print(
                                "Standard gold futures detected. Using 1 contract max in Evaluation Mode."
                            )
                        elif gold_symbol == "MGC":
                            print(
                                "Micro gold detected. Allowing scaling 1-3 contracts."
                            )
        ...
        print(
            "⚠️ Gold symbol not confirmed. Sentinel halted trading. Please check platform or preferences."
        )
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    def _place_trade(self, driver):
        ...
        try:
            ...
            try:
                WebDriverWait(driver, 30).until(
                    EC.presence_of_element_located(
                        (
                            By.XPATH,
                            "//div[contains(@class, 'chart')] | //div[contains(text(), 'Order')] | //div[contains(@class, 'order-panel')]",
                        )
                    )
                )
                ...
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                screenshot_path = (
                    f"{self.screenshot_dir}/bulenox_interface_not_found_{timestamp}.png"
                )
                driver.save_screenshot(screenshot_path)
            ...
            if not symbol_input:
                ...
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                screenshot_path = (
                    f"{self.screenshot_dir}/bulenox_no_symbol_input_{timestamp}.png"
                )
                driver.save_screenshot(screenshot_path)
                raise Exception("Symbol search input not found")
            ...
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            screenshot_path = (
                f"{self.screenshot_dir}/bulenox_symbol_search_{timestamp}.png"
            )
            driver.save_screenshot(screenshot_path)
            ...
            try:
                ...
                WebDriverWait(driver, 10).until(
                    EC.element_to_be_clickable(
                        (By.XPATH, f"//div[contains(text(), '{trading_symbol}')]")
                    )
                ).click()
                ...
                try:
                    ...
                    WebDriverWait(driver, 5).until(
                        EC.element_to_be_clickable(
                            (
                                By.XPATH,
                                "//div[contains(@class, 'search-results')]/div[1] | //div[contains(@class, 'dropdown')]/div[1]",
                            )
                        )
                    ).click()
            ...
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            screenshot_path = (
                f"{self.screenshot_dir}/bulenox_symbol_selected_{timestamp}.png"
            )
            driver.save_screenshot(screenshot_path)
            ...
            if quantity_input:
                ...
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                screenshot_path = (
                    f"{self.screenshot_dir}/bulenox_no_quantity_input_{timestamp}.png"
                )
                driver.save_screenshot(screenshot_path)
            ...
            if self.stopLoss:
                try:
                    sl_input = driver.find_element(
                        By.XPATH, "//input[@placeholder='Stop Loss']"
                    )
                    sl_input.clear()
            ...
            if self.takeProfit:
                try:
                    tp_input = driver.find_element(
                        By.XPATH, "//input[@placeholder='Take Profit']"
                    )
                    tp_input.clear()
            ...
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            screenshot_path = (
                f"{self.screenshot_dir}/bulenox_pre_submit_{timestamp}.png"
            )
            driver.save_screenshot(screenshot_path)
            ...
            try:
                ...
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                screenshot_path = (
                    f"{self.screenshot_dir}/bulenox_button_error_{timestamp}.png"
                )
                driver.save_screenshot(screenshot_path)
                raise
            ...
            try:
                WebDriverWait(driver, 10).until(
                    EC.presence_of_element_located(
                        (
                            By.XPATH,
                            "//div[contains(@class, 'confirmation-dialog')] | //div[contains(@class, 'modal')] | //div[contains(text(), 'Confirm')]",
                        )
                    )
                )
                ...
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                screenshot_path = (
                    f"{self.screenshot_dir}/bulenox_confirm_error_{timestamp}.png"
                )
                driver.save_screenshot(screenshot_path)
                raise
            ...
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            screenshot_path = (
                f"{self.screenshot_dir}/bulenox_trade_success_{timestamp}.png"
            )
            driver.save_screenshot(screenshot_path)

            print(
                f"Trade executed successfully: {self.signal['side']} {self.signal['quantity']} {trading_symbol}"
            )
            return True
        except Exception as e:
        ...
        try:
            ...
            if is_gold_trade:
                ...
                if not detected_symbol:
                    ...
                    return {
                        "status": "fail",
                        "message": "Gold symbol not confirmed. Trading halted.",
                        "gold_symbol_detected": False,
                    }
                ...
                if detected_symbol == "GC" and self.evaluation_mode:
                    ...
                    if original_quantity > 1:
                        print(
                            f"🕵️ Sentinel has detected symbol: {detected_symbol}. Executing 1 contract under {mode_str}."
                        )
                        ...
                        print(
                            f"🕵️ Sentinel has detected symbol: {detected_symbol}. Executing {original_quantity} contract under {mode_str}."
                        )
                    ...
                    if original_quantity > 3 and self.evaluation_mode:
                        print(
                            f"🕵️ Sentinel has detected symbol: {detected_symbol}. Limiting to 3 contracts under {mode_str}."
                        )
                        ...
                        print(
                            f"🕵️ Sentinel has detected symbol: {detected_symbol}. Executing {original_quantity} contracts under {mode_str}."
                        )

                elif detected_symbol == "XAUUSD":
                    # XAUUSD - use pip-based logic (CFD-style)
                    print(
                        f"🕵️ Sentinel has detected symbol: {detected_symbol}. Using pip-based logic (CFD-style) under {mode_str}."
                    )
