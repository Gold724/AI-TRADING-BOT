...
def calculate_pnl(trades):
    ...
    for trade in trades:
        ...
        if action == "entry" or action == "reentry":
            ...
            action == "partial_exit" or action == "take_profit" or action == "stop_loss"
        ):
            # Remove from position and calculate PnL
            if positions[symbol][opposite_type]["quantity"] > 0:
                # Close opposite position
                exit_quantity = min(
                    quantity, positions[symbol][opposite_type]["quantity"]
                )
                avg_entry = (
                    positions[symbol][opposite_type]["value"]
                    / positions[symbol][opposite_type]["quantity"]
                )
...
def generate_fibonacci_report(
    trade_history_file, output_file=None, include_visualization=True
):
    """
    ...
    for symbol, count in sorted(
        ...
        report += (
            f"- **{symbol}:** {count} trades ({count / total_trades * 100:.2f}%)\n"
        )
    ...
    if include_visualization:
        ...
        ax2.bar(
            [x[0] for x in sides_sorted],
            [x[1] for x in sides_sorted],
            color=["green" if x[0].lower() == "buy" else "red" for x in sides_sorted],
        )
        ...
        if fib_level_counts:
            fib_levels_sorted = sorted(
                fib_level_counts.items(),
                key=lambda x: (
                    float(x[0])
                    if x[0] != "Unknown"
                    and x[0] != "initial"
            ...
            ax4.bar(
                [x[0] for x in fib_levels_sorted], [x[1] for x in fib_levels_sorted]
            )
