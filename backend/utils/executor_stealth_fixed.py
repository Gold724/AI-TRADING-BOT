...
class StealthExecutor(BaseExecutor):
    ...
    def _init_driver(self):
        ...
        try:
            ...
            if chromedriver_path and os.path.exists(chromedriver_path):
                ...
                print(
                    "Chromedriver path not found or invalid. Using default chromedriver."
                )
                driver = webdriver.Chrome(options=chrome_options)

            # Execute CDP commands to add additional stealth
            if self.stealth_level >= 2 and not (
                self.profile_path and self.profile_name
            ):
        ...
        try:
            ...
            if any(
                ...
                if (
                    ...
                    and "signin" not in driver.current_url
                ):
                    print(
                        f"Already logged in - detected from URL: {driver.current_url}"
                    )
                    return True
            ...
            if "bulenox.com" in driver.current_url:
                try:
                    ...
                    result = driver.execute_script(
                        js_login, self.username, self.password
                    )
                    ...
                    if any(
                        ...
                        if (
                            ...
                            print(
                                f"JavaScript login successful - detected from URL: {driver.current_url}"
                            )
                    ...
                    print(
                        "JavaScript login approach didn't succeed, trying direct form submission"
                    )
                    ...
                    try:
                        ...
                        if any(
                            ...
                            if (
                                ...
                                print(
                                    f"Direct login successful - detected from URL: {driver.current_url}"
                                )
                                self._save_screenshot(driver, "bulenox_login_success")
                                return True
                    except Exception as e:
            ...
            if self.profile_path and self.profile_name:
                try:
                    ...
                    if (
                        "login" not in driver.current_url
                        and "sign-in" not in driver.current_url
                    ):
                        ...
                        if len(account_elements) > 0:
                            print(
                                f"Already logged in via Chrome profile - found {len(account_elements)} account elements"
                            )
                            return True
                    ...
                    auth_cookies = [
                        ...
                        if "auth" in cookie.get("name", "").lower()
                        or "session" in cookie.get("name", "").lower()
                    ]
                    if auth_cookies:
                        print(
                            f"Found authentication cookies: {[c.get('name') for c in auth_cookies]}"
                        )
                        # Try navigating to dashboard or trading page
                        try:
                            dashboard_url = self.broker_url.replace(
                                "login", "dashboard"
                            )
                            print(
                                f"Attempting to navigate to dashboard: {dashboard_url}"
                            )
                            driver.get(dashboard_url)
                            time.sleep(3)
                            if "login" not in driver.current_url:
                                print(
                                    "Successfully navigated to dashboard - already logged in"
                                )
                                return True
                        except Exception as e:
            ...
            for locator_type, locator_value in login_form_locators:
                try:
                    ...
                    print(
                        f"Login form found using locator: {locator_type}={locator_value}"
                    )
            ...
            if not login_form_found:
                print(
                    "Login form not found using standard locators. Taking screenshot..."
                )
                ...
                try:
                    ...
                    if len(input_fields) >= 2:
                        print(
                            f"Found {len(input_fields)} input fields, will attempt to use them"
                        )
                        login_form_found = True
                    else:
            ...
            try:
                ...
                if len(input_fields) >= 2:
                    ...
                    if not email_field:
                        for i, field in enumerate(input_fields):
                            ...
                            print(
                                f"Input #{i+1}: type={field_type}, id={field_id}, name={field_name}, class={field_class}"
                            )
                    ...
                    if not login_button:
                        ...
                        for i, button in enumerate(buttons):
                            ...
                            print(
                                f"Button #{i+1}: text={button.text}, type={button_type}, id={button_id}, class={button_class}"
                            )

                            if (
                                "login" in button_text
                                or "sign in" in button_text
                                or "log in" in button_text
                                or button_type == "submit"
                                or "login" in button_id.lower()
                                or "submit" in button_id.lower()
                            ):
                                login_button = button
                                ...
                                i == 0 and buttons
                            ):  # If no better match, use first button
                ...
                if (
                    ...
                    if len(all_inputs) >= 2:
                        ...
                        if (
                            not email_field
                            and all_inputs[0].get_attribute("type") != "password"
                        ):
                        ...
                        if all_buttons and not login_button:
                            login_button = all_buttons[
                                0
                            ]  # Assume first button is login
            ...
            print(
                f"Using username: {self.username[:3]}{'*' * (len(self.username) - 3)}"
            )
            ...
            try:
                ...
                while time.time() - start_time < wait_time:
                    try:
                        ...
                        if (
                            ...
                            and not "login" in current_url.lower()
                        ):
                            ...
                            for pattern in success_url_patterns:
                                if pattern in current_url.lower():
                                    print(
                                        f"Login successful - redirected to URL containing '{pattern}'"
                                    )
                                    login_success = True
                                    break
                        ...
                        for xpath in success_element_patterns:
                            try:
                                ...
                                if elements and any(e.is_displayed() for e in elements):
                                    print(
                                        f"Login successful - found element matching: {xpath}"
                                    )
                                    login_success = True
                                    break
                            except Exception as e:
                        ...
                        if auth_cookies:
                            print(
                                f"Found {len(auth_cookies)} authentication-related cookies"
                            )
                            ...
                            if "login" in current_url.lower():
                                try:
                                    ...
                                    driver.get(
                                        driver.current_url.split("/login")[0]
                                        + "/dashboard"
                                    )
                                    time.sleep(2)
                                    if "login" not in driver.current_url.lower():
                                        print(
                                            "Successfully navigated to dashboard after finding auth cookies"
                                        )
                                        login_success = True
                                        break
                                except:
                            ...
                            try:
                                ...
                                for error in error_elements:
                                    if error.is_displayed() and error.text.strip():
                                        ...
                                        if any(
                                            temp in error_text.lower()
                                            for temp in temporary_errors
                                        ):
                                            print(
                                                "Detected temporary error, will continue waiting"
                                            )
                                        else:
                                            return False
                            except Exception as e:
                    ...
                    try:
                        ...
                        if any(
                            term in page_title.lower() for term in success_title_terms
                        ):
                            print(
                                f"Login successful - page title indicates success: {page_title}"
                            )
                            login_success = True
                            break
                    except Exception as e:
                ...
                if not login_success:
                    ...
                    if (
                        ...
                        print(
                            "URL changed from login page and not on login page - assuming success"
                        )
                        login_success = True
    ...
    def _place_trade(self, driver):
        ...
        try:
                ...
                try:
                    WebDriverWait(driver, 5).until(
                        EC.presence_of_element_located(
                            (
                                By.XPATH,
                                f"//div[contains(text(), '{self.signal['symbol']}')]",
                            )
                        )
                    )
                    symbol_option = driver.find_element(
                        By.XPATH, f"//div[contains(text(), '{self.signal['symbol']}')]"
                    )
            ...
            button_id = (
                "buyButton" if self.signal["side"].lower() == "buy" else "sellButton"
            )
            ...
            WebDriverWait(driver, 10).until(
                EC.presence_of_element_located(
                    (By.XPATH, "//div[contains(text(), 'Trade executed successfully')]")
                )
            )
