...
class FibonacciExecutor(StealthExecutor):
    ...
    def _validate_signal(self):
        ...
        if "direction" in self.signal:
            self.signal["side"] = (
                "buy" if self.signal["direction"].lower() == "long" else "sell"
            )
        ...
        try:
            ...
            price_element = WebDriverWait(driver, 10).until(
                EC.presence_of_element_located(
                    (By.XPATH, "//div[contains(@class, 'current-price')]")
                ),
    ...
    def _close_partial_position(self, driver, percent):
        ...
        try:
            ...
            WebDriverWait(driver, 15).until(
                EC.presence_of_element_located(
                    (By.XPATH, "//div[contains(@class, 'positions-table')]")
                ),
            ...
            position_row = WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable(
                    (
                        By.XPATH,
                        f"//div[contains(@class, 'position-row') and contains(., '{self.signal['symbol']}')]",
                    )
                ),
            ...
            WebDriverWait(driver, 10).until(
                EC.presence_of_element_located(
                    (By.XPATH, "//div[contains(@class, 'position-details')]")
                ),
            ...
            close_button = driver.find_element(
                By.XPATH, "//button[contains(text(), 'Close Position')]"
            )
            ...
            WebDriverWait(driver, 10).until(
                EC.presence_of_element_located(
                    (By.XPATH, "//div[contains(@class, 'confirmation-dialog')]")
                ),
            ...
            driver.save_screenshot(
                f"{self.screenshot_dir}/fibonacci_partial_close_{timestamp}.png"
            )

            print(
                f"🔒 Closed {percent * 100:.0f}% of position ({close_quantity} units)"
            )
            return True
        except Exception as e:
            ...
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            driver.save_screenshot(
                f"{self.screenshot_dir}/fibonacci_close_error_{timestamp}.png"
            )
            return False
    ...
    def _regenerate_signal_from_current(self, driver, last_tp_level):
        ...
        if is_long:
            ...
            new_signal["fib_high"] = last_tp_level + (
                last_tp_level - new_signal["fib_low"]
            )
        else:
            ...
            new_signal["fib_low"] = last_tp_level - (
                new_signal["fib_high"] - last_tp_level
            )

        print(f"♻️ Regenerating new signal after pullback...")
        print(
            f"🎯 New Entry: {new_signal['entry']}, Direction: {'Long' if is_long else 'Short'}"
        )
    ...
    def _monitor_and_execute_fibonacci_strategy(self, driver):
        ...
        try:
            ...
            WebDriverWait(driver, 15).until(
                EC.presence_of_element_located(
                    (By.XPATH, "//div[contains(@class, 'trading-interface')]")
                ),
            ...
            is_long = self.signal["side"].lower() == "buy"
            print(
                f"🎯 Entry: {self.entry_price}, Direction: {'Long' if is_long else 'Short'}"
            )
            ...
            while time.time() - start_time < monitoring_duration:
                ...
                for i, target in enumerate(self.fib_targets):
                    ...
                    if (is_long and current_price >= target) or (
                        ...
                        close_success = self._close_partial_position(
                            driver, self.tp_percentages[i]
                        )
                        if close_success:
                ...
                if any(self.executed_levels):
                    ...
                    last_tp_index = max(
                        i for i, hit in enumerate(self.executed_levels) if hit
                    )
                    ...
                    if self._price_rejects_and_retests(current_price, last_tp_level):
                        ...
                        new_signal = self._regenerate_signal_from_current(
                            driver, last_tp_level
                        )
            ...
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            driver.save_screenshot(
                f"{self.screenshot_dir}/fibonacci_strategy_error_{timestamp}.png"
            )
            return False
        ...
        try:
            ...
            WebDriverWait(driver, 15).until(
                EC.presence_of_element_located(
                    (
                        By.XPATH,
                        "//div[contains(@class, 'trading-interface') or contains(@id, 'trade')]",
                    )
                ),
                message="Trading interface did not load in time",
            )
            ...
            symbol_input = WebDriverWait(driver, 10).until(
                EC.presence_of_element_located(
                    (
                        By.XPATH,
                        "//input[@name='symbol' or contains(@placeholder, 'Symbol')]",
                    )
                )
            )
            symbol_input.clear()
            ...
            quantity_input = driver.find_element(
                By.XPATH,
                "//input[@name='quantity' or contains(@placeholder, 'Quantity')]",
            )
            quantity_input.clear()
            ...
            if entry_price:
                entry_input = driver.find_element(
                    By.XPATH,
                    "//input[@name='entry' or contains(@placeholder, 'Entry')]",
                )
                entry_input.clear()
            ...
            if stop_loss:
                stop_loss_input = driver.find_element(
                    By.XPATH,
                    "//input[@name='stopLoss' or contains(@placeholder, 'Stop Loss')]",
                )
            ...
            if take_profit:
                take_profit_input = driver.find_element(
                    By.XPATH,
                    "//input[@name='takeProfit' or contains(@placeholder, 'Take Profit')]",
                )
            ...
            place_button = driver.find_element(
                By.XPATH,
                "//button[contains(text(), 'Place Trade') or contains(text(), 'Submit')]",
            )
            place_button.click()
            ...
            WebDriverWait(driver, 10).until(
                EC.presence_of_element_located(
                    (
                        By.XPATH,
                        "//div[contains(@class, 'success') or contains(text(), 'Order placed')]",
                    )
                )
            )
