...
class QuantConnectIntegration:
    def __init__(self):
        ...
        if not self.api_key or not self.project_id:
            raise ValueError(
                "QuantConnect API key and project ID must be set in .env file"
            )
    ...
    def _make_request(self, endpoint, method="GET", data=None):
        ...
        # Create auth data
        auth_data = {"apiKey": self.api_key, "timestamp": timestamp}
    ...
    def get_live_algorithm_status(self, deployment_id):
        ...
        data = {"deployId": deployment_id}
        return self._make_request(endpoint, method="POST", data=data)

    def deploy_live_algorithm(
        self, project_id=None, compile=True, node_id=None, brokerage="Paper"
    ):
    ...
    def stop_live_algorithm(self, deployment_id):
        ...
        data = {"deployId": deployment_id, "status": "Stopped"}
        return self._make_request(endpoint, method="POST", data=data)
