...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/auto_recovery.log"), logging.StreamHandler()],
)
...
class AutoRecoveryEngine:
    ...
    def __init__(
        ...
        logger.info(
            f"Auto Recovery Engine initialized with {len(self.sessions)} sessions"
        )
    ...
    def _load_history(self) -> None:
        ...
        try:
            if os.path.exists(self.history_file):
                ...
                logger.info(
                    f"Loaded {len(self.recovery_history)} recovery events from history file"
                )
        except Exception as e:
        ...
        try:
            with self.lock:
                if session_id in self.sessions:
                    ...
                    logger.warning(
                        f"Session {session_id} not found for heartbeat update"
                    )
                    return False
    ...
    def _check_session(self, session_id: str, session_info: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if heartbeat_age > self.heartbeat_interval * 2:
                logger.warning(
                    f"Session {session_id} heartbeat is {heartbeat_age:.1f}s old, checking health"
                )
                ...
                if health_check and callable(health_check):
                    ...
                    try:
                        ...
                        logger.error(
                            f"Health check failed for session {session_id}: {str(e)}"
                        )
                    ...
                    if not is_healthy:
                        ...
                        logger.warning(
                            f"Session {session_id} is unhealthy (failure {session_info['failure_count']}/{self.max_failures})"
                        )
                    ...
                    logger.error(
                        f"Health check function not found for session {session_id}"
                    )
        ...
        try:
            ...
            if recovery_func and callable(recovery_func):
                ...
                try:
                    ...
                    error_message = str(e)
                    logger.error(
                        f"Recovery function failed for session {session_id}: {error_message}"
                    )
                ...
                if success:
                    ...
                    self._recovery_failure(
                        session_id, session_info, recovery_event, error_message
                    )
            else:
    ...
    def get_recovery_history(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            return (
                self.recovery_history[-limit:]
                if limit > 0
                else self.recovery_history.copy()
            )
        except Exception as e:
...
def log_notification(recovery_event: Dict[str, Any]) -> None:
    ...
    if status == "success":
        logger.info(
            f"NOTIFICATION: Successfully recovered session {session_id} for broker {broker_id}"
        )
    else:
        error = recovery_event.get("error", "Unknown error")
        logger.error(
            f"NOTIFICATION: Failed to recover session {session_id} for broker {broker_id}: {error}"
        )
