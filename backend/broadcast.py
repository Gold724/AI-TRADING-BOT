...
class SignalBroadcaster:
    ...
    def add_subscriber(
        ...
        logger.info(
            f"Added subscriber: {subscriber_id} for broker {broker_id} and account {account_id}"
        )
        return True
    ...
    def broadcast_signal(self, signal: Dict[str, Any], source: str = "manual"):
        ...
        signal_with_meta["broadcast_id"] = (
            f"bcast-{int(time.time())}-{len(self.signal_history) + 1}"
        )
        ...
        for subscriber in self.subscribers:
            ...
            if subscriber_id in self.filters and not self.filters[subscriber_id](
                signal
            ):
        ...
        logger.info(
            f"Signal broadcast complete: {successful_deliveries} successful, {failed_deliveries} failed"
        )
    ...
    def _deliver_signal(
        ...
        try:
            ...
            if response.status_code in (200, 201, 202):
                ...
                logger.error(
                    f"Failed to deliver signal to {subscriber['id']}: {response.status_code} {response.text}"
                )
                failed_deliveries += 1
    ...
    def get_signal_history(self, limit: int = 50, filter_func: Callable = None):
        ...
        if filter_func and callable(filter_func):
            filtered_history = [
                signal for signal in self.signal_history if filter_func(signal)
            ]
...
def create_filter(filter_config: Dict[str, Any]):
    ...
    def filter_func(signal):
        # Symbol filter
        if (
            "symbols" in filter_config
            and signal.get("symbol") not in filter_config["symbols"]
        ):
            return False

        # Side filter
        if (
            "sides" in filter_config
            and signal.get("side") not in filter_config["sides"]
        ):
            return False

        # Min quantity filter
        if (
            "min_quantity" in filter_config
            and signal.get("quantity", 0) < filter_config["min_quantity"]
        ):
            return False

        # Max quantity filter
        if (
            "max_quantity" in filter_config
            and signal.get("quantity", 0) > filter_config["max_quantity"]
        ):
            return False

        # Source filter
        if (
            "sources" in filter_config
            and signal.get("source") not in filter_config["sources"]
        ):
            return False

        # Custom filter function
        if "custom_filter" in filter_config and callable(
