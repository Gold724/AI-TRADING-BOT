...
def run_test(test_type, signal_index, signals_file, **kwargs):
    ...
    if test_type == "simulation":
        cmd = [
            "python",
            "simulate_fibonacci_strategy.py",
            "--index",
            str(signal_index),
            "--file",
            signals_file,
            "--time",
            str(kwargs.get("time", 60)),
            "--interval",
            str(kwargs.get("interval", 1)),
            "--output",
            f"logs/simulated_fibonacci_trades_{signal_index}.json",
        ]
    elif test_type == "api":
        cmd = [
            "python",
            "api_fibonacci_strategy.py",
            "--index",
            str(signal_index),
            "--file",
            signals_file,
            "--url",
            kwargs.get("url", "http://localhost:5000/api/trade/fibonacci"),
        ]
    elif test_type == "executor":
        cmd = [
            "python",
            "test_fibonacci_executor.py",
            "--symbol",
            kwargs.get("symbol", "EURUSD"),
            "--side",
            kwargs.get("side", "buy"),
            "--quantity",
            str(kwargs.get("quantity", 0.1)),
            "--entry",
            str(kwargs.get("entry", 1.0850)),
            "--fib_low",
            str(kwargs.get("fib_low", 1.0800)),
            "--fib_high",
            str(kwargs.get("fib_high", 1.0900)),
            "--stealth_level",
            str(kwargs.get("stealth_level", 2)),
        ]
...
def run_batch_tests(
    test_type, signals_file="sample_fibonacci_signals.json", indices=None, **kwargs
):
    """
    ...
    if indices is None:
        ...
        if not indices:
            print(
                f"No valid indices provided. Please choose indices between 0 and {len(signals)-1}"
            )
            return False
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="simulation",
        choices=["simulation", "api", "executor"],
        help="Test type",
    )
    parser.add_argument(
        "--file",
    ...
    parser.add_argument(
        "--indices", type=str, help="Comma-separated list of signal indices to test"
    )
    parser.add_argument(
        "--time",
        type=int,
        default=60,
        help="Simulation time in seconds (for simulation tests)",
    )
    parser.add_argument(
        "--interval",
        type=int,
        default=1,
        help="Simulation interval in seconds (for simulation tests)",
    )
    parser.add_argument(
        "--url",
        type=str,
        default="http://localhost:5000/api/trade/fibonacci",
        help="API endpoint URL (for API tests)",
    )
    parser.add_argument(
        "--stealth_level",
        type=int,
        default=2,
        choices=[1, 2, 3],
        help="Stealth level (for executor tests)",
    )
    parser.add_argument(
    ...
    if args.list:
        ...
        if signals:
            ...
            for i, signal in enumerate(signals):
                print(
                    f"[{i}] {signal.get('symbol')} {signal.get('side')} - {signal.get('description')}"
                )
            print(
