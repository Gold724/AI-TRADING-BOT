...
class TestFibonacciDashboard(unittest.TestCase):
    ...
    def test_add_signal(self):
        ...
        response = requests.post(
            urljoin(self.base_url, "/api/add-signal"), json=test_signal
        )
        ...
        for signal in signals:
            if (
                signal.get("symbol") == test_signal["symbol"]
                and signal.get("entry") == test_signal["entry"]
                and signal.get("description") == test_signal["description"]
            ):
                found = True
                break
