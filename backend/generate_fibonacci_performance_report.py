...
def calculate_position_metrics(df):
                    ...
                    if (
                        ...
                        pos["pl_pct"] = (
                            ...
                            * (
                            ...
                            + pl_pct * trade["quantity"]
                        ) / (pos["initial_quantity"] - pos["remaining_quantity"])
                        pos["pl_amount"] += pl_amount

                        # Update max excursions
                        pos["max_favorable_excursion"] = max(
                            pos["max_favorable_excursion"], pl_pct if pl_pct > 0 else 0
                        )
                        pos["max_adverse_excursion"] = min(
                            pos["max_adverse_excursion"], pl_pct if pl_pct < 0 else 0
                        )
    ...
    if positions:
        ...
        positions_df["fib_levels_hit"] = positions_df["fib_levels_hit"].apply(
...
def generate_summary_metrics(df, positions_df):
    ...
    metrics["most_traded_symbol"] = (
        symbol_counts.index[0] if not symbol_counts.empty else "None"
    )
    metrics["most_traded_symbol_count"] = (
        symbol_counts.iloc[0] if not symbol_counts.empty else 0
    )
    ...
    metrics["most_common_fib_level"] = (
        fib_level_counts.index[0] if not fib_level_counts.empty else 0
    )
    metrics["most_common_fib_level_count"] = (
        fib_level_counts.iloc[0] if not fib_level_counts.empty else 0
    )

    # Calculate average Fibonacci level
    metrics["avg_fib_level"] = (
        df["fib_level"].mean() if "fib_level" in df.columns else 0
    )

    # Position metrics
    if not positions_df.empty:
        metrics["total_positions"] = len(positions_df)
        metrics["closed_positions"] = len(
            positions_df[positions_df["status"] == "closed"]
        )
        ...
        if not closed_positions.empty:
            ...
            metrics["profitable_positions"] = len(
                closed_positions[closed_positions["pl_amount"] > 0]
            )
            ...
            metrics["max_consecutive_wins"] = max_consecutive(
                closed_positions.sort_values("exit_time")["pl_amount"] > 0
            )
...
def generate_performance_report(history_file, output_dir):
    ...
    print(
        f"Open {os.path.join(output_dir, 'fibonacci_performance_report.html')} to view the report"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate Fibonacci Strategy Performance Report"
    )
    parser.add_argument(
        "--history_file",
