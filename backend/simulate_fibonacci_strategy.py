...
def calculate_fib_targets(fib_low, fib_high, is_long):
    ...
    for level in FIB_LEVELS:
        target = (
            fib_low + (fib_high - fib_low) * level
            if is_long
            else fib_high - (fib_high - fib_low) * level
        )
...
def get_simulated_price(
    entry, fib_low, fib_high, is_long, time_elapsed, volatility=0.2
):
    """
...
def simulate_fibonacci_strategy(
    signal, max_time=300, interval=5, output_file="logs/simulated_fibonacci_trades.json"
):
    """
    ...
    try:
        while elapsed_time < max_time and remaining_quantity > 0:
            ...
            # Get simulated price
            current_price = get_simulated_price(
                entry, fib_low, fib_high, is_long, elapsed_time
            )

            # Check for stop loss
            if stop_loss and (
                (is_long and current_price <= stop_loss)
                or (not is_long and current_price >= stop_loss)
            ):
                # Log stop loss
                stop_loss_trade = {
                    "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            ...
            for i, target in enumerate(fib_targets):
                ...
                if (is_long and current_price >= target) or (
                    ...
                    # Check for reentry
                    if remaining_quantity > 0 and price_rejects_and_retests(
                        current_price, target, is_long
                    ):
            ...
            if int(elapsed_time) % 10 == 0 and int(elapsed_time) > 0:
                print(
                    f"⏱️ {elapsed_time:.1f}s: 🔍 Current price: {current_price}, Remaining: {remaining_quantity}"
                )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Index of the signal to use from the signals file",
    )
    parser.add_argument(
        "--file",
        type=str,
    ...
    parser.add_argument(
        "--time", type=int, default=300, help="Maximum simulation time in seconds"
    )
    parser.add_argument(
        "--interval", type=int, default=5, help="Simulation interval in seconds"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="logs/simulated_fibonacci_trades.json",
        help="Path to save the trade history",
    )
    parser.add_argument(
    ...
    if args.index < 0 or args.index >= len(signals):
        print(
            f"Invalid signal index. Please choose an index between 0 and {len(signals)-1}"
        )
        return
