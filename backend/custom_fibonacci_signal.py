...
def run_custom_fibonacci_signal(
    symbol,
    side,
    quantity,
    entry,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run Fibonacci Strategy with a custom signal"
    )
    ...
    parser.add_argument(
        "--fib_low", type=float, required=True, help="Fibonacci low price"
    )
    parser.add_argument(
        "--fib_high", type=float, required=True, help="Fibonacci high price"
    )
    ...
    parser.add_argument(
        ...
        default=2,
        choices=[1, 2, 3],
        help="Stealth level (1-3)",
    )
    parser.add_argument(
        "--save", action="store_true", help="Save custom signal to file"
    )
    parser.add_argument(
        "--description", type=str, help="Description of the custom signal"
    )
    ...
    if args.description is not None:
        ...
        signal["description"] = (
            f"{args.symbol} {args.side.upper()} at {args.entry} with Fibonacci range {args.fib_low}-{args.fib_high}"
        )
