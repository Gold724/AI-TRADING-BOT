...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/multi_broker.log"), logging.StreamHandler()],
)
...
class MultiBrokerManager:
    ...
    def __init__(
        ...
        logger.info(
            f"Multi-Broker Manager initialized with {len(self.sessions)} sessions"
        )
        ...
        try:
            if os.path.exists(self.sessions_file):
                ...
                for session_id, session_data in sessions_data.items():
                    ...
                    session.created_at = session_data.get(
                        "created_at", session.created_at
                    )
                    session.last_activity = session_data.get(
    ...
    def create_session(
        self, broker_id: str, account_id: str = None, metadata: Dict[str, Any] = None
    ) -> str:
                ...
                if session_id in self.sessions:
                    ...
                    if self.sessions[session_id].driver:
                        try:
                            ...
                            logger.warning(
                                f"Error closing driver for session {session_id}: {str(driver_e)}"
                            )
    ...
    def get_trades(
        ...
        if session_id:
            ...
            if session_id in self.sessions:
                all_trades = (
                    self.sessions[session_id].trades[-limit:]
                    if limit > 0
                    else self.sessions[session_id].trades
                )
        else:
    ...
    def register_with_recovery_engine(self, session_id: str, recovery_engine) -> bool:
        ...
        try:
            ...
            if not session:
                logger.warning(
                    f"Session {session_id} not found for recovery registration"
                )
                return False

            if session.recovery_registered:
                logger.warning(
                    f"Session {session_id} already registered with recovery engine"
                )
                return True
            ...
            success = recovery_engine.register_session(
                ...
                metadata={"account_id": session.account_id, **session.metadata},
            )
        ...
        try:
            ...
            if not target_session and broker_id:
                ...
                if sessions:
                    ...
                    active_sessions = [
                        s for s in sessions if s.status not in ["closed", "error"]
                    ]
                    if active_sessions:
            ...
            if not trade_id:
                return {"success": False, "error": "Failed to add trade to session"}
...
def create_example_sessions():
    ...
    exness_session_id = broker_manager.create_session(
        broker_id="exness",
        account_id="EX123456",
        metadata={"currency": "USD", "leverage": 1000, "platform": "MT5"},
    )
    ...
    binance_session_id = broker_manager.create_session(
        broker_id="binance",
        account_id="BI789012",
        metadata={"currency": "USDT", "futures": True, "platform": "Binance Futures"},
    )
    ...
    bulenox_session_id = broker_manager.create_session(
        broker_id="bulenox",
        account_id="BU345678",
        metadata={"currency": "EUR", "leverage": 500, "platform": "Proprietary"},
    )
