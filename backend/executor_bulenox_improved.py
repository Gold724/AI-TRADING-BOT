...
class BulenoxExecutor:
    def __init__(self):
        ...
        self.chrome_driver_path = os.getenv(
            "CHROME_DRIVER_PATH",
            "D:\\aibot\\chromedriver-win64\\chromedriver-win64\\chromedriver.exe",
        )
        ...
        if not self.username or not self.password:
            raise ValueError(
                "BULENOX_USERNAME and BULENOX_PASSWORD environment variables must be set"
            )
    ...
    def _init_driver(self):
        ...
        while driver is None and retry_count < max_retries:
            retry_count += 1
            self.logger.info(
                f"Driver initialization attempt {retry_count}/{max_retries}"
            )
            ...
            try:
                ...
                if os.path.exists(self.user_data_dir):
                    self.logger.info(
                        f"Attempting to initialize with user profile: {self.profile_directory}"
                    )
                    ...
                    try:
                        ...
                        self.logger.info(
                            "Successfully initialized Chrome with user profile"
                        )
                        break
                    except Exception as e:
                        self.logger.warning(
                            f"Failed to initialize with user profile: {str(e)}"
                        )
                ...
                if driver is None:
                    ...
                    try:
                        ...
                        self.logger.info(
                            "Successfully initialized Chrome without user profile"
                        )
                        break
                    except Exception as e:
                        self.logger.warning(
                            f"Failed to initialize without user profile: {str(e)}"
                        )
                        # Continue to next approach
                ...
                if driver is None:
                    ...
                    try:
                        ...
                        self.logger.info(
                            "Successfully initialized Chrome with minimal options"
                        )
                        break
                    except Exception as e:
                        self.logger.warning(
                            f"Failed to initialize with minimal options: {str(e)}"
                        )
                        # Will retry on next iteration

            except Exception as e:
                self.logger.error(
                    f"Unexpected error during driver initialization: {str(e)}"
                )
        ...
        try:
            ...
            for cookie in cookies:
                try:
                    ...
                    self.logger.warning(
                        f"Could not add cookie {cookie.get('name')}: {str(e)}"
                    )
    ...
    def _login(self):
        ...
        try:
            ...
            initial_screenshot = os.path.join(
                self.screenshots_dir, f"bulenox_initial_{timestamp}.png"
            )
            ...
            try:
                captcha_elements = self.driver.find_elements(
                    By.XPATH,
                    "//div[contains(@class, 'captcha') or contains(@class, 'g-recaptcha')]",
                )
                if captcha_elements:
                    self.logger.error(f"CAPTCHA detected on login page")
                    for element in captcha_elements:
                        self.logger.error(
                            f"CAPTCHA element: {element.get_attribute('outerHTML')}"
                        )
            except Exception as e:
            ...
            if not username_field:
                ...
                try:
                    ...
                    for i, input_field in enumerate(inputs):
                        try:
                            self.logger.info(
                                f"Input {i+1}: type={input_field.get_attribute('type')}, name={input_field.get_attribute('name')}, id={input_field.get_attribute('id')}"
                            )
                        except:
                            pass
                except Exception as e:
            ...
            if not password_field:
                ...
                raise NoSuchElementException(
                    "Password field not found with any selector"
                )
            ...
            try:
                ...
                hidden_fields = form_element.find_elements(
                    By.XPATH, ".//input[@type='hidden']"
                )
                if hidden_fields:
                    self.logger.info(
                        f"Found {len(hidden_fields)} hidden fields in login form"
                    )
                    for field in hidden_fields:
                        ...
                        if (
                            ...
                            self.logger.info(
                                f"Found important security token: {name}={value}"
                            )
            except Exception as e:
            ...
            submit_selectors = [
                ...
                (
                    By.XPATH,
                    "//button[contains(text(), 'Sign in') or contains(text(), 'Login') or contains(text(), 'SIGN IN') or contains(text(), 'LOG IN')]",
                ),
                (
                    By.XPATH,
                    "//input[contains(@value, 'Sign in') or contains(@value, 'Login')]",
                ),
            ...
            for by_method, selector in submit_selectors:
                try:
                    ...
                    self.logger.info(
                        f"Found submit button with selector: {by_method}={selector}"
                    )
                    break
                except NoSuchElementException:
                    continue
            ...
            if not submit_button:
                ...
                try:
                    ...
                    for i, button in enumerate(buttons):
                        try:
                            self.logger.info(
                                f"Button {i+1}: text='{button.text}', type={button.get_attribute('type')}"
                            )
                        except:
                            pass
                except Exception as e:
                ...
                raise NoSuchElementException(
                    "Submit button not found with any selector"
                )

            # Take pre-submit screenshot
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            pre_submit_screenshot = os.path.join(
                self.screenshots_dir, f"bulenox_pre_submit_{timestamp}.png"
            )
            ...
            try:
                ...
                for error in error_elements:
                    if error.is_displayed() and error.text.strip():
                        self.logger.warning(
                            f"Pre-submission error message found: {error.text}"
                        )
            except Exception as e:
                self.logger.warning(
                    f"Could not check for pre-submission error messages: {str(e)}"
                )
            ...
            error_screenshot = os.path.join(
                self.screenshots_dir, f"bulenox_login_error_{timestamp}.png"
            )
            try:
                ...
                self.logger.error(
                    f"Could not save error screenshot: {str(screenshot_error)}"
                )
    ...
    def _wait_for_login_success(self):
        ...
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        post_submit_screenshot = os.path.join(
            self.screenshots_dir, f"bulenox_post_submit_{timestamp}.png"
        )
        ...
        while time.time() - wait_start < wait_timeout:
            ...
            if any(url_part in current_url for url_part in self.success_url_patterns):
                ...
                success_screenshot = os.path.join(
                    self.screenshots_dir, f"bulenox_login_success_{timestamp}.png"
                )
        ...
        final_screenshot = os.path.join(
            self.screenshots_dir, f"bulenox_login_final_{timestamp}.png"
        )
        ...
        if login_status == "success" or any(
            url_part in self.driver.current_url
            for url_part in self.success_url_patterns
        ):
            self.logger.info("Login successful")
            return True
        elif login_status == "failed" or any(
            url_part in self.driver.current_url
            for url_part in self.failure_url_patterns
        ):
            ...
            self.logger.warning(
                f"Login result unclear - final URL: {self.driver.current_url}"
            )
            return False
        ...
        try:
            ...
            WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located(
                    (
                        By.XPATH,
                        "//div[contains(@class, 'trading') or contains(@class, 'platform')]",
                    )
                ),
            ...
            trading_screenshot = os.path.join(
                self.screenshots_dir, f"bulenox_trading_{timestamp}.png"
            )
            ...
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            error_screenshot = os.path.join(
                self.screenshots_dir, f"bulenox_trade_error_{timestamp}.png"
            )
            try:
                self.driver.save_screenshot(error_screenshot)
                ...
                self.logger.error(
                    f"Could not save trade error screenshot: {str(screenshot_error)}"
                )
