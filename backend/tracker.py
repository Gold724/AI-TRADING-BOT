...
class CompoundingTracker:
    ...
    def calculate_daily_target(self, day: int, mode: str = MODE_SAFE) -> float:
        ...
        if mode == MODE_FAST:
            ...
            return FAST_PASS_TARGETS.get(
                day, 3500
            )  # Default to the last day's target if out of range
        elif mode == MODE_SAFE:
            # Safe Growth Mode - conservative targets
            return SAFE_GROWTH_TARGETS.get(
                day, 2500
            )  # Default to the last day's target if out of range
        else:
            # Default to original targets for backward compatibility
            return DAILY_TARGETS.get(
                day, 3100
            )  # Default to the last day's target if out of range
    ...
    def calculate_contracts(
        ...
        if mode == MODE_FAST:
            ...
            base_contracts = round(
                MAX_CONTRACTS * day_factor * drawdown_factor * aggression_multiplier
            )
        else:
            ...
            base_contracts = round(
                MAX_CONTRACTS * day_factor * drawdown_factor * 0.8
    ...
    def log_trade(
        self, pnl: float, contracts_used: int, timestamp: Optional[str] = None
    ) -> Dict:
        ...
        # Update day's trades
        trade = {"pnl": pnl, "contracts": contracts_used, "timestamp": timestamp}
        ...
        if day_data["actual_pnl"] >= day_data["daily_target"]:
            ...
            day_data["actual_pnl"] < 0
            and abs(day_data["actual_pnl"]) >= DAILY_LOSS_LIMIT
        ):
    ...
    def calculate_per_trade_targets(
        self, daily_target: float, num_trades: int = 3
    ) -> float:
    ...
    def generate_trade_plan(
        self, day: Optional[int] = None, num_trades: int = 4, mode: str = MODE_SAFE
    ) -> Dict:
        ...
        suggested_contracts = self.calculate_contracts(
            day, self.current_state["remaining_drawdown"], mode
        )
        ...
        max_daily_loss = min(
            DAILY_LOSS_LIMIT * max_daily_loss_factor, remaining_drawdown
        )
        ...
        return {
            ...
            "stop_loss_per_contract": min(
                100,
                max(
                    55, remaining_drawdown * 0.01 * (0.8 if mode == MODE_SAFE else 1.2)
                ),
            ),
            "take_profit_per_contract": min(
                100,
                max(
                    55, remaining_drawdown * 0.01 * (0.8 if mode == MODE_SAFE else 1.2)
                ),
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            "progress_percentage": min(
                100, (self.current_state["total_pnl"] / MAX_PROFIT_TARGET) * 100
            ),
...
def generate_trade_plan(
    day: Optional[int] = None, num_trades: int = 4, mode: str = MODE_SAFE
) -> Dict:
