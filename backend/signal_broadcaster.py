...
class SignalBroadcaster:
    ...
    def __init__(
        ...
        logger.info(
            f"Signal Broadcaster initialized with {len(self.subscribers)} subscribers"
        )
        ...
        try:
            ...
            for existing in self.subscribers:
                if existing["id"] == subscriber["id"]:
                    logger.warning(
                        f"Subscriber with ID {subscriber['id']} already exists, updating"
                    )
            ...
            self._save_subscribers()
            logger.info(
                f"Added new subscriber: {subscriber['name']} ({subscriber['broker_id']})"
            )
            return True
    ...
    def broadcast_signal(self, signal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for subscriber in self.subscribers:
                ...
                if "filters" in subscriber and not self._passes_filters(
                    enriched_signal, subscriber["filters"]
                ):
                    results["filtered"] += 1
            ...
            logger.info(
                f"Signal {signal_id} broadcast complete: {results['successful']} successful, {results['failed']} failed, {results['filtered']} filtered"
            )
            return results

        except Exception as e:
            logger.error(f"Error broadcasting signal: {str(e)}")
            return {"error": str(e), "successful": 0, "failed": 0, "filtered": 0}
    ...
    def _deliver_signal(
        ...
        try:
            ...
            if response.status_code >= 200 and response.status_code < 300:
                ...
                logger.info(
                    f"Signal delivered successfully to {subscriber['name']} ({subscriber['broker_id']})"
                )
            else:
                ...
                with threading.Lock():
                    ...
                    results["details"].append(detail)
                logger.error(
                    f"Failed to deliver signal to {subscriber['name']} ({subscriber['broker_id']}): {response.status_code}"
                )
            ...
            with threading.Lock():
                ...
                results["details"].append(detail)
            logger.error(
                f"Request error delivering signal to {subscriber['name']} ({subscriber['broker_id']}): {str(e)}"
            )
            ...
            with threading.Lock():
                ...
                results["details"].append(detail)
            logger.error(
                f"Error delivering signal to {subscriber['name']} ({subscriber['broker_id']}): {str(e)}"
            )
    ...
    def _passes_filters(self, signal: Dict[str, Any], filters: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Quantity filter
            if (
                "min_quantity" in filters
                and signal.get("quantity", 0) < filters["min_quantity"]
            ):
                return False
            if (
                "max_quantity" in filters
                and signal.get("quantity", 0) > filters["max_quantity"]
            ):
                return False
