...
def get_backtests():
    ...
    try:
        ...
        return (
            jsonify(
                {"status": "error", "message": f"Error retrieving backtests: {str(e)}"}
            ),
            500,
        )
...
def get_backtest(backtest_id):
    ...
    try:
        ...
        return (
            jsonify(
                {"status": "error", "message": f"Error retrieving backtest: {str(e)}"}
            ),
            500,
        )
...
def create_backtest():
    ...
    try:
        data = request.json
        name = data.get(
            "name", f'Sentinel Backtest {datetime.now().strftime("%Y-%m-%d %H:%M")}'
        )
        ...
        if not backtest_id:
            return (
                jsonify({"status": "error", "message": "Failed to create backtest"}),
                500,
            )
        ...
        return (
            jsonify(
                {"status": "error", "message": f"Error creating backtest: {str(e)}"}
            ),
            500,
        )
...
def stop_live():
    ...
    try:
        ...
        if not success:
            return (
                jsonify(
                    {"status": "error", "message": "Failed to stop live algorithm"}
                ),
                500,
            )
...
def get_signals():
    ...
    try:
        ...
        return jsonify(
            {
                ...
                "performance": (
                    {
                        "drawdown": results.get("RuntimeStatistics", {}).get(
                            "Drawdown", "N/A"
                        ),
                        "total_return": results.get("RuntimeStatistics", {}).get(
                            "TotalReturn", "N/A"
                        ),
                    }
                    if results
        ...
        return (
            jsonify(
                {"status": "error", "message": f"Error retrieving signals: {str(e)}"}
            ),
            500,
        )
