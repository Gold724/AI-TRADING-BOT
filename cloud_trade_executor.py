...
# Import the AI-powered login function and executor
...
class CloudTradeExecutor:
    def __init__(self, account_id=None, session_id=None):
        ...
        self.session_id = (
            session_id
            or f"{self.account_id}-{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"
        )
        ...
        self.driver = None
        self.logger.info(
            f"CloudTradeExecutor initialized for account {self.account_id} with session {self.session_id}"
        )
        self.logger.info(
            f"Running in {'headless' if self.headless else 'visible'} mode"
        )
    ...
    def login(self):
        ...
        try:
            self.logger.info(
                f"Attempting to login to Bulenox for account {self.account_id}"
            )
            ...
            failure_count = profile_failures.get(preferred_profile, {}).get(
                "failures", 0
            )
            last_failure_time = profile_failures.get(preferred_profile, {}).get(
                "last_failure"
            )
            last_success_time = profile_failures.get(preferred_profile, {}).get(
                "last_success"
            )

            self.logger.info(
                f"Profile {preferred_profile} has {failure_count} recorded failures"
            )
            ...
            if failure_count > 2:
                self.logger.warning(
                    f"Profile {preferred_profile} has failed {failure_count} times, trying fallback"
                )

                # Always reset cookies for the failed profile
                try:
                    self.logger.info(
                        f"Resetting cookies for {preferred_profile} to re-authenticate fresh"
                    )
                ...
                for fallback in fallback_profiles:
                    fallback_failures = profile_failures.get(fallback, {}).get(
                        "failures", 0
                    )
                    fallback_last_success = profile_failures.get(fallback, {}).get(
                        "last_success"
                    )
                ...
                if not profile_switched:
                    self.logger.warning(
                        f"No viable fallback profiles found. Using original profile {preferred_profile} with reset cookies"
                    )
            ...
            # Call AI-powered login function with parameters matching the function definition
            self.driver = ai_login_bulenox(
                debug=True
            )
            ...
            if self.driver:
                ...
                if preferred_profile in profile_failures:
                    profile_failures[preferred_profile]["failures"] = (
                        profile_failures[preferred_profile].get("failures", 0) + 1
                    )
                else:
                    profile_failures[preferred_profile] = {
                        "failures": 1,
    ...
    def execute_trade(self, signal):
        ...
        try:
            ...
            self.logger.info(
                f"Executing {side} trade for {quantity} {futures_symbol} (original: {symbol})"
            )
            ...
            try:
                ...
                self.logger.info(
                    "Waiting for UI to fully load using enhanced detection..."
                )
                ...
                try:
                    WebDriverWait(self.driver, 15).until(
                        lambda d: d.execute_script(
                            "return document.querySelector('.order-module') !== null"
                        )
                    )
                    self.logger.info("Order module detected in DOM via JavaScript")
                except Exception as e:
                    self.logger.warning(
                        f"Order module not detected with JS: {e}, trying alternative selectors"
                    )
                    ...
                    try:
                        ...
                        self.logger.info(
                            "Alternative trading UI elements detected via JavaScript"
                        )
                    except Exception as alt_e:
                        self.logger.warning(
                            f"Failed to detect trading UI elements via JavaScript: {alt_e}"
                        )
                        ...
                        try:
                            WebDriverWait(self.driver, 15).until(
                                EC.presence_of_element_located(
                                    (
                                        By.XPATH,
                                        "//div[contains(@class, 'order-module') or contains(@class, 'trade-form') or contains(@class, 'trading-panel')]",
                                    )
                                )
                            )
                            ...
                            self.logger.error(
                                f"Failed to detect trading UI elements via XPath: {xpath_e}"
                            )
                ...
                try:
                    ...
                    if time_sales_present:
                        self.logger.info(
                            "'Time and Sales' tab detected via JavaScript, UI appears fully loaded"
                        )
                    else:
                        ...
                        try:
                            time_sales_element = WebDriverWait(self.driver, 10).until(
                                EC.presence_of_element_located(
                                    (
                                        By.XPATH,
                                        "//div[contains(text(), 'Time and Sales') or contains(@class, 'time-sales')]",
                                    )
                                )
                            )
                            ...
                            self.logger.warning(
                                f"'Time and Sales' tab not detected via XPath: {ts_xpath_e}"
                            )
                except Exception as e:
                    self.logger.warning(
                        f"Could not check for 'Time and Sales' tab via JavaScript: {e}"
                    )
                ...
                try:
                    ...
                    for js_check in js_balance_checks:
                        ...
                        if account_balance:
                            self.logger.info(
                                f"Account balance from JS: {account_balance}"
                            )
                            break

                    if not account_balance:
                        self.logger.warning(
                            "Could not retrieve account balance via any JS method"
                        )
                except Exception as e:
                ...
                try:
                    ...
                    if not last_signal:
                        self.logger.warning(
                            "Could not retrieve last signal via any JS method"
                        )
                except Exception as e:
            ...
            try:
                ...
                for selector in symbol_selectors:
                    try:
                        ...
                        self.logger.info(
                            f"Found symbol input using selector: {selector}"
                        )
                        break
                    except Exception:
                        continue
                ...
                try:
                    dropdown_item = WebDriverWait(self.driver, 5).until(
                        EC.element_to_be_clickable(
                            (By.XPATH, f"//div[contains(text(), '{futures_symbol}')]")
                        ),
                    )
                    dropdown_item.click()
                    ...
                    self.logger.warning(
                        f"No dropdown selection needed or available: {e}"
                    )
                ...
                for selector in quantity_selectors:
                    try:
                        ...
                        self.logger.info(
                            f"Found quantity input using selector: {selector}"
                        )
                        break
                    except Exception:
                        continue
                ...
                if stop_loss:
                    ...
                    for selector in stop_loss_selectors:
                        try:
                            ...
                            self.logger.info(
                                f"Found stop loss input using selector: {selector}"
                            )
                            break
                        except Exception:
                            continue
                ...
                if take_profit:
                    ...
                    for selector in take_profit_selectors:
                        try:
                            ...
                            self.logger.info(
                                f"Found take profit input using selector: {selector}"
                            )
                            break
                        except Exception:
                            continue
                ...
                self.logger.info(
                    f"Found {button_count} {button_text} buttons on the page"
                )
                ...
                try:
                    button = self.driver.find_element(
                        By.XPATH,
                        f"(//div[contains(@class, 'order-module') or contains(@class, 'trade-form')]//button[contains(text(), '{button_text}')])[1]",
                    )
                    self.logger.info(f"Found {button_text} button in Order module")
                except Exception:
                    self.logger.warning(
                        f"Could not find {button_text} button in Order module"
                    )
                    ...
                    try:
                        ...
                        for selector in sibling_selectors:
                            try:
                                ...
                                self.logger.info(
                                    f"Found {button_text} button with sibling context: {selector}"
                                )
                                break
                            except Exception:
                                continue
                        ...
                        self.logger.warning(
                            f"Error finding {button_text} button with sibling context: {e}"
                        )
                ...
                if not button and button_count > 0:
                    try:
                        ...
                        button = self.driver.find_element(
                            By.XPATH,
                            f"(//button[contains(text(), '{button_text}')])[1]",
                        )
                        self.logger.info(f"Found {button_text} button by index")
                    except Exception:
                        self.logger.warning(
                            f"Could not find {button_text} button by index"
                        )
                    ...
                    try:
                        ...
                        for selector in class_selectors:
                            try:
                                button = self.driver.find_element(By.XPATH, selector)
                                self.logger.info(
                                    f"Found {button_text} button with class selector: {selector}"
                                )
                                break
                            except Exception:
                                continue
                        ...
                        self.logger.warning(
                            f"Error finding {button_text} button with class selectors: {e}"
                        )
                ...
                if not button:
                    try:
                        ...
                        self.logger.info(
                            f"Found {button_text} button with generic approach"
                        )
                    except Exception as e:
                        self.logger.error(
                            f"Could not find {button_text} button with any approach: {e}"
                        )
                ...
                try:
                    button.click()
                    self.logger.info(
                        f"Clicked {button_text} button with standard click"
                    )
                    ...
                    try:
                        ...
                        self.logger.info(
                            f"Clicked {button_text} button with JavaScript click"
                        )
                    except Exception as js_error:
                        self.logger.error(f"JavaScript click also failed: {js_error}")
                        raise Exception(
                            f"Failed to click {button_text} button: {js_error}"
                        )
                ...
                try:
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"Standard confirm click failed: {confirm_click_error}. Trying JavaScript click..."
                        )
                        self.driver.execute_script(
                    ...
                    self.logger.info(
                        f"No confirmation dialog appeared or error handling it: {e}"
                    )
                ...
                self.logger.info(
                    f"Successfully executed {side} trade for {quantity} {futures_symbol}"
                )
                ...
                self.log_trade_status(
                    signal, "failed", f"Trade execution error: {str(e)}"
                )
                self._log_emotional_feedback(False)
                return False
...
def main():
    ...
    executor = CloudTradeExecutor(
        account_id=args.account_id, session_id=args.session_id
    )
