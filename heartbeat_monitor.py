    ...
    try:
        ...
        with open(HEARTBEAT_STATUS_FILE, "r") as f:
            ...
            status_data = {
                ...
                "timestamp": (
                    lines[1] if len(lines) >= 2 else datetime.now().isoformat()
                ),
                "session_active": False,
                "file_exists": True,
            }
...
def monitor_heartbeat(
    interval_seconds=60, max_age_minutes=5, notify=True, restart_cmd=None
):
    """
    ...
    """
    logger.info(
        f"Starting heartbeat monitor (interval={interval_seconds}s, max_age={max_age_minutes}m)"
    )
    ...
    while True:
        try:
            ...
            is_healthy, status_data, age_minutes = check_heartbeat_health(
                max_age_minutes
            )
            ...
            if is_healthy != last_health:
                if is_healthy:
                    message = (
                        f"[OK] Heartbeat is now HEALTHY - Status: {current_status}"
                    )
                    logger.info(message)
                    consecutive_unhealthy = 0
                    if notify and SLACK_AVAILABLE:
                        send_slack_notification(
                            message, notification_type="heartbeat_healthy"
                        )
                else:
                    ...
                    if notify and SLACK_AVAILABLE:
                        send_slack_notification(
                            message, notification_type="heartbeat_unhealthy"
                        )
            ...
            if not is_healthy:
                ...
                if consecutive_unhealthy % 5 == 0:
                    logger.warning(
                        f"Heartbeat still unhealthy after {consecutive_unhealthy} checks"
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Monitor the AI Trading Sentinel heartbeat"
    )
    parser.add_argument(
        "--interval",
        "-i",
    ...
    parser.add_argument(
        "--no-notify", "-n", action="store_true", help="Disable Slack notifications"
    )
    parser.add_argument(
        "--restart",
        "-r",
    ...
    parser.add_argument(
        "--check", "-c", action="store_true", help="Just check current status and exit"
    )
    ...
    if args.check:
        ...
        print(
            f"Session Active: {'[OK] Yes' if status_data.get('session_active', False) else '[ERROR] No'}"
        )
        return
