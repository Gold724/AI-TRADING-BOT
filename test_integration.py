...
def test_dashboard_api():
    ...
    test_data = {
        ...
        "brokers": {"bulenox": {"status": "active", "session_count": 1}},
        "strategies": {
            "active_strategy": "fibonacci_strategy",
            "available_strategies": ["fibonacci_strategy", "default_strategy"],
        },
        "signals": {"total_signals": 10, "processed_signals": 8, "pending_signals": 2},
        "health": {
...
def test_strategy_mutation():
    ...
    ), f"Strategy available API failed: {response.text}"
    data = response.json()
    ...
    mutation_data = {
        ...
        "changes": {"stop_loss_pct": 0.6, "take_profit_pct": 1.8},
    }
...
def test_broadcast_integration():
    ...
    ), f"Signal not registered in dashboard: {data}"
...
def test_auto_recovery():
    ...
    data = response.json()
    assert (
        data["data"]["overall_status"] == "recovering"
    ), f"Health status not updated: {data}"
