...
HEARTBEAT_STATUS_FILE = os.path.join(
...
def pull_updates():
    ...
    try:
        ...
        result = subprocess.run(
            ["git", "pull"], cwd=repo_root, check=True, capture_output=True, text=True
        )

        update_heartbeat_status(
            "updated",
            {"message": "Successfully pulled updates", "details": result.stdout},
        )
...
def clone_repository(target_dir):
    ...
    try:
        ...
        update_heartbeat_status(
            "cloned",
            {"message": "Successfully cloned repository", "target_dir": target_dir},
        )
        return True, result.stdout
...
def push_changes(commit_message):
    ...
    try:
        ...
        push_result = subprocess.run(
            ["git", "push"], cwd=repo_root, check=True, capture_output=True, text=True
        )
...
def create_github_issue(title, body, labels=None):
    ...
    try:
        ...
        update_heartbeat_status(
            "error", {"error": f"Failed to create GitHub issue: {str(e)}"}
        )
        return False, f"Error: {str(e)}"
...
def sync_trading_results(results_file):
    ...
    try:
        ...
        update_heartbeat_status(
            "results_synced",
            {"message": "Successfully synced trading results", "file": relative_path},
        )
