...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("bulenox_login_test.log")],
)
    ...
    try:
        ...
        try:
            ...
            if captcha_elements:
                ...
                for element in captcha_elements:
                    logger.error(
                        f"[TEST] CAPTCHA element: {element.get_attribute('outerHTML')}"
                    )
        ...
        for by_method, selector in username_selectors:
            try:
                ...
                logger.info(
                    f"[TEST] Found username field with selector: {by_method}={selector}"
                )
                break
        ...
        for by_method, selector in password_selectors:
            try:
                ...
                logger.info(
                    f"[TEST] Found password field with selector: {by_method}={selector}"
                )
                break
            except NoSuchElementException:
                continue
        ...
        try:
            ...
            if hidden_fields:
                logger.info(
                    f"[TEST] Found {len(hidden_fields)} hidden fields in login form"
                )
        ...
        if not username or not password:
            logger.error(
                "[TEST] Missing credentials. Set BULENOX_USERNAME and BULENOX_PASSWORD environment variables."
            )
            return False
        ...
        for by_method, selector in submit_selectors:
            try:
                ...
                logger.info(
                    f"[TEST] Found submit button with selector: {by_method}={selector}"
                )
                break
            except NoSuchElementException:
                continue
        ...
        while time.time() - wait_start < wait_timeout:
            ...
            if any(url_part in current_url for url_part in failure_url_patterns):
                ...
                try:
                    ...
                    logger.warning(
                        f"[TEST] Could not check for error messages: {str(e)}"
                    )
        ...
        logger.info(
            f"[TEST] Final login status screenshot saved to: {final_screenshot}"
        )

        # Determine login success based on final URL and status
        if login_status == "success" or any(
            url_part in driver.current_url for url_part in success_url_patterns
        ):
            logger.info("[TEST] Direct login test PASSED")
            return True
        elif login_status == "failed" or any(
            url_part in driver.current_url for url_part in failure_url_patterns
        ):
            logger.error("[TEST] Direct login test FAILED - still on login page")
            return False
        else:
            logger.warning(
                f"[TEST] Direct login test result UNCLEAR - final URL: {driver.current_url}"
            )
            return not any(
                url_part in driver.current_url for url_part in failure_url_patterns
            )
...
def test_cookie_based_login():
    ...
    try:
        ...
        if "login" in driver.current_url.lower():
            logger.error(
                "[TEST] Cookie-based login test FAILED - redirected to login page"
            )
            return False
        else:
