...
def test_bulenox_selenium_dev():
    ...
    try:
        ...
        if "trading" in driver.current_url or "dashboard" in driver.current_url:
            ...
            if not username or not password:
                ...
                try:
                    ...
                    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                    screenshot_path = (
                        f"{screenshot_dir}/bulenox_after_login_{timestamp}.png"
                    )
                    driver.save_screenshot(screenshot_path)
                    print(f"Screenshot saved to: {screenshot_path}")
        ...
        try:
            ...
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            screenshot_path = (
                f"{screenshot_dir}/bulenox_symbol_selected_{timestamp}.png"
            )
            driver.save_screenshot(screenshot_path)
            print(f"Screenshot saved to: {screenshot_path}")
        ...
        try:
            ...
            if symbol_input:
                ...
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                screenshot_path = (
                    f"{screenshot_dir}/bulenox_gold_entered_{timestamp}.png"
                )
                driver.save_screenshot(screenshot_path)
                print(f"Screenshot saved to: {screenshot_path}")
                ...
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                screenshot_path = (
                    f"{screenshot_dir}/bulenox_gold_selected_{timestamp}.png"
                )
                driver.save_screenshot(screenshot_path)
                print(f"Screenshot saved to: {screenshot_path}")
            ...
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            screenshot_path = (
                f"{screenshot_dir}/bulenox_gold_test_error_{timestamp}.png"
            )
            driver.save_screenshot(screenshot_path)
            print(f"Error screenshot saved to: {screenshot_path}")
